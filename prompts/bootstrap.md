...
2. Add the kit files into this project. **Merge, do not replace:**
   - If `knowledge-base/` already exists (from an existing agentic system or prior setup), only add the kit's specific files (`00-master.md`, `01-setup-run.md`, `02-test-writing.md`) into it. Do not overwrite or remove any existing files in the directory.
   - If `knowledge-base/` does not exist, create it and copy all kit KB files into it.
   - Same for `agents/` — only add the kit's agent files (`cypress-setup-agent.md`, `cypress-test-writer-agent.md`). Do not overwrite or remove existing agent files.
